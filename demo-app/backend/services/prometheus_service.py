...
PROMETHEUS_QUERY_RANGE_API = f"{PROMETHEUS_URL}/api/v1/query_range"

class PrometheusService:
    def __init__(self, prometheus_url: str = PROMETHEUS_URL):
        self.prometheus_url = prometheus_url
        self.query_api = f"{prometheus_url}/api/v1/query"
        self.query_range_api = f"{prometheus_url}/api/v1/query_range"
    ...
    async def execute_query_range(self, query: str, start: float, end: float, step: str = "15s"):
        async with httpx.AsyncClient() as client:
            try:
                params = {
                    "query": query,
                ...
                response = await client.get(self.query_range_api, params=params, timeout=8.0)
                if response.status_code != 200:
                    return None
                return response.json()
            except Exception as e:
                print(f"[PrometheusService] query_range error: {e}")
                return None
    ...
    async def get_cpu_metrics(self):
        ...
        query = 'sum(rate(container_cpu_usage_seconds_total{pod!=""}[1m])) by (pod)'
        try:
    ...
    async def get_summary_metrics(self):
        cpu_query = 'sum(rate(container_cpu_usage_seconds_total{pod!=""}[1m])) by (pod)'
