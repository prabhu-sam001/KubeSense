...
"""
    ...
    candidate_urls = [
        ...
        "http://172.16.128.157:11434",
        "http://172.31.192.1:11434",
    ...
    async with httpx.AsyncClient() as client:
        for url in candidate_urls:
            try:
                print(f"[Recommendation Engine] Requesting Ollama ({url}) using model '{OLLAMA_MODEL}'...")
                ...
                if response.status_code == 200:
                    ...
                    print(f"[Recommendation Engine] Ollama response from {url}: {response_text[:120]}...")
                    ...
                    if explanation and (recommended_fixes or preventive_measures):
                        used_llm = True
                        print("[Recommendation Engine] LLM recommendation generated successfully via Llama 3.1!")
                        break
                    else:
                        print("[Recommendation Engine] Warning: Ollama response structure invalid.")
                else:
                    print(f"[Recommendation Engine] Ollama API at {url} returned status code {response.status_code}.")
            except Exception as e:
                print(f"[Recommendation Engine] Endpoint {url} unreachable: {e}")
                continue
