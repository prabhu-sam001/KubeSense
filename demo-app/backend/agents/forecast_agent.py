...
async def _run_memory_forecast(prometheus: PrometheusService, db):
    """Predict working-set memory pressure for every active pod."""
    try:
        metrics = await prometheus.get_memory_metrics()
    except Exception as e:
        print(f"[ForecastAgent] Memory metrics fetch error: {e}")
        return
    ...
    for item in metrics:
        ...
        _append_window(_memory_history, pod, memory_mb)
        history = _memory_history[pod]
        if len(history) < MIN_SAMPLES_REQUIRED:
            continue

        # Use a modest headroom allowance above the current rolling baseline.
        # This keeps the prediction meaningful even when no resource limit exists.
        threshold = max(history) * 1.5
        fc = _fit_and_forecast(history, threshold, POLL_INTERVAL_SECONDS)
        if fc["slope"] <= 1e-9:
            continue

        svc_label = pod.split("-")[0]
        minutes = fc["minutes_to_breach"]
        msg = (
            f"Memory Pressure Forecast: {svc_label} working set is trending upward "
            f"(slope: +{fc['slope']:.2f} MiB/interval, R²={fc['r_squared']:.2f}, "
            f"current {memory_mb:.1f} MiB). "
            + (f"Estimated to exceed {threshold:.1f} MiB in ≈{minutes} min."
               if minutes else
               f"No threshold breach expected within {MAX_FORECAST_MINUTES} min.")
        )
        await _persist_forecast(
            db, "memory", svc_label, fc["current"], threshold, fc, msg, _severity(minutes)
        )
...
async def run_forecast_agent():
    ...
    while True:
        ...
        try:
            ...
            await _run_memory_forecast(prometheus, db)
