...
export const apiService = {
  ...
  getMetricsHistory: async (timeframe: string = '15m'): Promise<{ timeframe: string; count: number; data: any[]; latest: any }> => {
    try {
      const { data } = await api.get(`/api/metrics/history?timeframe=${timeframe}`);
      return data;
    } catch (err) {
      console.warn('Failed to fetch metrics history:', err);
      return { timeframe, count: 0, data: [], latest: null };
    }
  },
};
