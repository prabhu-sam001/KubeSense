...
export function DependencyGraph({ topology, telemetry }: DependencyGraphProps) {
  ...
  useEffect(() => {
    ...
    const getLoadPercent = (nodeId: string): number => {
      ...
      const cpuLoad = Math.min(Math.round(totalCores * 100), 100);

      const totalMemMb = matchKeys.reduce(
        (sum, k) => sum + (metrics[k]?.memory_mb ?? 0), 0
      );
      const memLimit = name === 'prometheus' ? 2048 : 1024;
      const memLoad = Math.min(Math.round((totalMemMb / memLimit) * 100), 100);
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-surface-50 rounded-xl p-4">
                ...
                <p className="text-lg font-700 text-surface-900">{cpuInfo?.cpu_cores !== undefined ? `${Math.round(cpuInfo.cpu_cores * 1000)}m` : '—'}</p>
                <p className="text-xs text-surface-400">{cpuInfo?.cpu_cores?.toFixed(4) || '0'} cores</p>
              </div>
              <div className="bg-surface-50 rounded-xl p-4">
                ...
                <p className="text-lg font-700 text-surface-900">{cpuInfo?.memory_mb !== undefined ? `${cpuInfo.memory_mb.toFixed(0)}Mi` : '—'}</p>
                <p className="text-xs text-surface-400">RAM</p>
              </div>
              <div className="bg-surface-50 rounded-xl p-4">
